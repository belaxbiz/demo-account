...
module DemoAccount
  module UbiregiAPI
    class Client
      ...
      #  If given, the block will be yielded with each raw response converted to hash.
      #
      def checkouts(account_id, &block)
        _index("accounts/#{account_id}/checkouts", "checkouts")
      end
      ...
      #
      def post_checkouts(account_id, checkouts)
        _post("accounts/#{account_id}/checkouts", "checkouts" => checkouts)
      end
